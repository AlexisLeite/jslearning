...
export const ListadoCelulares = () => (
  <Box id="ListadoCelulares">
    <Heading as="h1">Listado de celulares</Heading>
    <Text>A continuación se muestra una lista de celulares:</Text>
    <ul>
      {Celulares.map(current => (
        <li key={current.marca}>
          Marca: <strong>{current.marca} </strong>- modelo:
          <strong>{current.modelo}</strong> - Precio:
          <strong>{current.precio}</strong>
        </li>
      ))}
    </ul>
  </Box>
